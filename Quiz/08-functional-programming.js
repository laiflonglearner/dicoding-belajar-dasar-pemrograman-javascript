...
function findProductById(products, id) {
   ...
   */
  return products.find((product) => product.id === id); // jangan pakai filter karena bakal return array
}
...
function applyDiscount(products, discount) {
  ...
  return products.map((product) => ({
    ...product, // salin semua properti produk (id, name, etc) biar ga hilang
    price: product.price - (product.price * discount) / 100, // harga udah didiskon
  }));
}
